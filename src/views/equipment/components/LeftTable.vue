      ...
      <el-table-column prop="status" label="状态">
        <template slot-scope="scope">
          <div
            ...
            style="line-height: 20px;margin: 0 auto;text-align: center;border-radius: 10px;width: 80px;height: 20px;background: #00c553;color:#fff"
          >已处理</div>
          <div
            v-if="scope.row.status == '未处理'"
            style="line-height: 20px;margin: 0 auto;text-align: center;border-radius: 10px;width: 80px;height: 20px;background: #ff0d0d;color:#fff"
          >未处理</div>
...
export default {
  ...
  watch: {
    arr() {
      this.tableData = this.arr.equipmentConditionVOList
    }
  },
  methods: {
    getRowClass() {
